defmodule Osumiex.Mqtt.Encoder do
  ...
  def encode(%Osumiex.Mqtt.Message.SubAck{message_type: message_type, message_id: message_id, topics: topics}) do
    ...
    msg_id = message_id(message_id)
    len = byte_size(msg_id) + byte_size(qos_binary)

    <<encode_header(message_type, len) :: binary, message_id(message_id) :: binary, qos_binary :: binary>>
  end
  def encode(%Osumiex.Mqtt.Message.Publish{message_type: message_type, topic: topic, qos: qos, message: message})
    when qos == :fire_and_forget do
    topic = utf8(topic)
    msg_id = message_id(0)
